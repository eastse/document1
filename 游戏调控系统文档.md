...
func CalculateWeightStreamResult(rtpType string, request *entity.AdjustRequest) (*entity.WeightResult, error) {
	// 1. 强制选择 中表
	rtpType = serverconst.DYNAMIC_RTP_HIGH

	// 2. 深拷贝
	weightOddsList := make([]entity.WeightOddsPair, len(request.AdjustConfig.PerformanceCtrConfig.RtpType2WeightOddsListMap[rtpType]))
	copy(weightOddsList, request.AdjustConfig.PerformanceCtrConfig.RtpType2WeightOddsListMap[rtpType])
    
	// 3. 触发奖励
	if (request.UserAdjust.UserAdjustControl.CurrentBetCount + 1) == request.UserAdjust.UserAdjustControl.CurrentFeatureBoardNumber {
		return calculatePerformanceResultAbsoluteFeature(weightOddsList, request.AdjustConfig.PerformanceCtrConfig.WeightPerformanceList)
	}
...
**calculatePerformanceResultAbsoluteFeature（触发奖励）**
...
func calculatePerformanceResultAbsoluteFeature(
	...
	// 1. 定義 開獎權重計算類型與特色權重率 對應表
	const FeatureOdds = 50.0
	const ZeroTotalWeight = 0
	const NonFeatureExceptZeroTotalWeight = 0
	const FeatureTotalWeight = 10000
    
	// 2. 計算特色倍數與非特色倍數陣列
	featureOddsIndexList, zeroOddsIndexList, nonFeatureOddsIndexList := getOddsIndexListByFeatureOddsAndTopN(weightOddsList, FeatureOdds)
